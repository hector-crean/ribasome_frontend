...
pub fn easy_mark(ui: &mut Ui, easy_mark: &str) {
    easy_mark_it(ui, easy_mark_parser::Parser::new(easy_mark));
}

pub fn easy_mark_it<'em>(ui: &mut Ui, items: impl Iterator<Item = easy_mark_parser::Item<'em>>) {
...
pub fn item_ui(ui: &mut Ui, item: easy_mark_parser::Item<'_>) {
    let row_height = ui.text_style_height(&TextStyle::Body);
    let one_indent = row_height / 2.0;

    match item {
        easy_mark_parser::Item::Newline => {
        ...
        easy_mark_parser::Item::Text(style, text) => {
        ...
        easy_mark_parser::Item::Hyperlink(style, text, url) => {
            let label = rich_text_from_style(text, &style);
            if style.small && !style.raised {
                ui.with_layout(Layout::left_to_right(Align::BOTTOM), |ui| {
        ...
        easy_mark_parser::Item::Separator => {
            ui.add(Separator::default().horizontal());
        }
        easy_mark_parser::Item::Indentation(indent) => {
            let indent = indent as f32 * one_indent;
            ui.allocate_exact_size(vec2(indent, row_height), Sense::hover());
        }
        easy_mark_parser::Item::QuoteIndent => {
            let rect = ui
...
fn rich_text_from_style(text: &str, style: &easy_mark_parser::Style) -> RichText {
    let easy_mark_parser::Style {
        heading,
        quoted,
        code,
        strong,
